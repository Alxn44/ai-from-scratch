...
export const LESSON_CONTENT: Record<number, Record<string, LessonText>> = {
  ...
  6: {
    es: {
      technical: 'El modelo no redacta una frase completa de una vez: genera el siguiente token y repite. Un token puede ser una palabra, una parte de una palabra o un signo. Con el texto que ya lleva, asigna una probabilidad a cada token posible. Si escribes «el café está muy…», podrían quedar así: «caliente» 31, «bueno» 22, «rico» 14, «frío» 9 y el resto repartido. Elige uno, lo añade al texto y recalcula con la frase ahora más larga. No guarda un plan de párrafo: la coherencia aparece porque cada token nuevo se decide teniendo en cuenta el contexto anterior.',
      ...
      examples: [
        ...
        { titulo: 'Token a token', entrada: 'escoge «caliente» y sigue', salida: 'ahora calcula sobre «el café está muy caliente…»', nota: 'Cada token cambia el reparto del siguiente.' },
      ],
    },
    en: {
      technical: 'The model does not draft a whole sentence at once: it generates the next token and repeats. A token can be a word, part of a word or a punctuation mark. From the text so far, it assigns a probability to every possible token. Type “the coffee is very…” and it might look like this: “hot” 31, “good” 22, “nice” 14, “cold” 9, with the rest spread thin. It picks one, appends it and recalculates with the now-longer sentence. It has no stored paragraph plan: coherence emerges because each new token is chosen with the preceding context in view.',
      ...
      examples: [
        ...
        { titulo: 'Token by token', entrada: 'it picks “hot” and carries on', salida: 'now it scores over “the coffee is very hot…”', nota: 'Every token changes the split for the next one.' },
      ],
    },
  },
  7: {
  ...
  8: {
    es: {
      technical: 'El modelo solo puede tener presente una cantidad limitada de texto a la vez; a ese límite se le llama ventana de contexto. Su tamaño cambia según el modelo y la aplicación, y se mide en tokens, no en palabras. Ahí caben las instrucciones del sistema, tu pedido, los archivos que pegaste y lo dicho antes. Cada respuesta nueva se calcula mirando ese bloque. Cuando se llena, la aplicación puede resumir, recortar o dejar fuera lo más antiguo. El modelo no «olvida» como una persona: simplemente deja de recibir ese texto. Por eso, en chats largos, puede repetir ideas o perder un detalle mencionado al principio.',
    ...
    en: {
      technical: 'A model can only keep a limited amount of text in view at once; that limit is called the context window. Its size changes by model and application, and it is measured in tokens, not words. It holds system instructions, your request, pasted files and what has already been said. Every new answer is calculated from that block. When it fills up, the application may summarise, trim or leave out the oldest material. The model does not “forget” like a person: it simply stops receiving that text. That is why long chats can repeat ideas or lose a detail from the beginning.',
  ...
  9: {
    es: {
      technical: 'La temperatura regula cuánto azar acepta el modelo al elegir el siguiente token. Baja, concentra la elección en las opciones más probables; alta, da más oportunidad a opciones menos probables y vuelve el texto menos predecible. No garantiza que una respuesta sea correcta ni hace que el modelo «sepa más». Tampoco existe un valor bueno para todo: conviene bajarla en datos, resúmenes, código y traducciones, cuando buscas respuestas consistentes; conviene subirla un poco en lluvia de ideas, nombres y textos creativos, cuando repetir la misma salida es el problema.',
    ...
    en: {
      technical: 'Temperature controls how much randomness the model accepts when choosing the next token. A low setting concentrates the choice on the most likely options; a high setting gives less likely options more of a chance and makes the text less predictable. It does not guarantee a correct answer or make the model “know more”. There is no good setting for everything: keep it low for facts, summaries, code and translation when you want consistent answers; raise it a little for brainstorming, names and creative writing when repetition is the problem.',
  ...
  10: {
    es: {
      technical: 'Un modelo puede decir «no sé», pero no siempre detecta que le falta información. Su trabajo es producir una continuación probable, y un dato inventado puede sonar más probable que una admisión de incertidumbre. Por eso puede generar nombres plausibles, fechas verosímiles, citas impecables o artículos que nunca existieron. No está mintiendo: mentir supone saber la verdad; el modelo está completando patrones. La regla práctica no es dejar de usarlo, sino verificar en la fuente todo dato que tenga consecuencias antes de actuar con él.',
    ...
    en: {
      technical: 'A model can say “I don’t know”, but it does not always detect that information is missing. Its job is to produce a likely continuation, and a made-up fact can sound more likely than an admission of uncertainty. That is why it can generate plausible names, believable dates, immaculate citations or articles that never existed. It is not lying: lying assumes knowing the truth; the model is completing patterns. The practical rule is not to stop using it, but to check the source before acting on any fact with consequences.',
  ...
  11: {
    es: {
      technical: 'El entrenamiento usa información disponible hasta una fecha concreta. Para lo que ocurrió después, el modelo no tiene una fuente actual por sí solo: puede dar una respuesta desactualizada como si fuera presente. Una aplicación puede conectarlo a una búsqueda o a documentos recientes: recupera una fuente y se la entrega junto con tu pregunta. El modelo sigue siendo el mismo; ahora tiene material actualizado para responder. Lo importante no es la seguridad con que lo diga, sino poder ver y abrir la fuente antes de confiar en un dato reciente.',
    ...
    en: {
      technical: 'Training uses information available up to a particular date. For what happened afterwards, the model has no current source on its own and may present an outdated answer as current. An application can connect it to search or recent documents: it retrieves a source and supplies it alongside your question. The model is still the same; it simply has current material to use. What matters is not how confidently it speaks, but whether you can see and open the source before trusting a recent fact.',
  ...
  12: {
    es: {
      technical: 'Con lo anterior ya tienes un modelo mental útil: aprende de ejemplos, guarda patrones en números, no se reentrena cuando le hablas, lee y genera tokens, responde según tu pedido, pierde lo que queda fuera del contexto, cambia con la temperatura, puede inventar y tiene una fecha de corte. Ahora falta practicar. Cinco minutos al día sobre trabajo real bastan para empezar: el correo que ibas a escribir, el texto que ibas a resumir o la duda que ibas a investigar. Pide qué necesitas, para quién y en qué formato; después verifica todo lo que tenga consecuencias.',
    ...
    en: {
      technical: 'You now have a useful mental model: it learns from examples, stores patterns as numbers, does not retrain when you talk to it, reads and generates tokens, responds to your request, loses what falls outside its context, changes with temperature, can invent and has a cutoff date. Now practise. Five minutes a day on real work is enough to start: the email you were going to write, the text you were going to summarise or the question you were going to research. Ask for what you need, who it is for and the format; then verify anything with consequences.',
