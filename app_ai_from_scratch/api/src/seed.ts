...
const REAL: Record<string, LabSeed> = {
...
// Returns the payload exactly as it is stored. Only touches `order` labs.
const storedPayload = (id: string, r: LabSeed): Record<string, unknown> => {
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    if (r) {
      await run(INS_LAB, [id, n, i, LEVELS[i - 1]!, r.kind, r.prompt,
        JSON.stringify(storedPayload(id, r)), JSON.stringify(r.solution), r.explanation, 0]);
      real++;
    } else {
